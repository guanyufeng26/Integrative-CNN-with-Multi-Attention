...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout, BatchNormalization, \
    GlobalAveragePooling2D, GlobalMaxPooling2D, Reshape, Multiply, Concatenate, Lambda, Layer
...
def cbam_channel_attention(input_feature, ratio=2):
    ...
    shared_dense = Dense(channel // ratio, activation='relu', kernel_initializer='he_normal', use_bias=False)(concat_pool)
    avg_dense = Dense(channel, activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(shared_dense)
    max_dense = Dense(channel, activation='sigmoid', kernel_initializer='he_normal', use_bias=False)(shared_dense)
...
def build_fusion_attention_model(params, input_shape=(74, 1, 1)):
    ...
    model = Model(inp, out)
    model.compile(Adam(params['lr']), 'categorical_crossentropy', metrics=['accuracy'])
    return model
...
model = build_fusion_attention_model(best_params)
es = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
model.fit(X_train, y_train, validation_data=(X_val, y_val_onehot), epochs=50, batch_size=32, callbacks=[es], verbose=0)
...
with open(os.path.join(output_dir, "CNN_triple_attention_val_metrics.json"), 'w', encoding='utf-8') as f:
