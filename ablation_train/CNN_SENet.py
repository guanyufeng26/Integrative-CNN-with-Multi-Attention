...
def se_block(input_feature, ratio=2):
    ...
    se_feature = Dense(channel,
                       ...
                       kernel_initializer='he_normal',
                       use_bias=False)(se_feature)
    se_output = Multiply()([input_feature, se_feature])
    return se_output
...
def build_cnn_se(params, input_shape):
    ...
    model = Model(inputs=inputs, outputs=outputs, name="CNN_74D_SE")
...
def load_best_params():
    original_params_path = os.path.join("./output/fusion_6_68", "best_hyperparameters.json")
...
def train_cnn_se():
    input_shape = (74, 1, 1)
    model = build_cnn_se(best_params, input_shape)
...
se_model, se_history = train_cnn_se()
...
y_val_pred_prob = se_model.predict(X_val, verbose=0)
y_val_pred = np.argmax(y_val_pred_prob, axis=1)
val_metrics = calculate_metrics(y_val, y_val_pred, y_val_pred_prob)

model_info = {
    "model_type": "CNN+SENet",
    "base_params_source": "original_74d_fusion_best_params",
    "senet_ratio": 2
}
with open(os.path.join(output_dir, "CNN+SENet_model_info.json"), 'w', encoding='utf-8') as f:
    json.dump(model_info, f, ensure_ascii=False, indent=4)

with open(os.path.join(output_dir, "CNN+SENet_val_metrics.json"), 'w', encoding='utf-8') as f:
...
save_pred_prob(val_ids, y_val, y_val_pred, y_val_pred_prob, "CNN+SENet_val_predictions.csv")

se_model.save(os.path.join(output_dir, "CNN+SENet_best_model.h5"))