...
def load_6d_data(file_name):
    ...
    cols = ['VV频率', 'SS频率', 'DD频率', 'HH频率', 'NN频率', 'FF频率']
    X = df[cols].values
    y = df['蛋白类型标签'].values
...
def build_cnn_cbam_global(params, input_shape=(74,1,1)):
...
model = build_cnn_cbam_global(best_params)
...
with open(os.path.join(output_dir, "CNN+CBAM+GlobalSelfAttention_val_metrics.json"), 'w', encoding='utf-8') as f:
